...
int get_count(line_node* line_list) {
  line_node* node = line_list;
  int count = 0;
  while (node != NULL) {
    node = node->next;
    count++;
...
void quadtree_insert_lines(quad_tree* tree, line_node* new_lines, double timeStep, int num_lines) {
  ...
  assert(num_lines == get_count(lines));
  assert(num_parent_lines + num_quad1 + num_quad2 + num_quad3 + num_quad4 == num_lines);
